...
`/palace:search`, a dependency-free keyword-overlap lookup (no embeddings). Feature depth lives
in spine docs under `<wing>/<repo>/<feature>/`, indexed by the generated `_features.md`; the same
search reads their `answers:` frontmatter. `features/{feature-slug}.md` is a legacy location, still
searched, where `/palace:update` adds nothing new. Neither is part of the 6-file completeness check.
...
- **Tests** — `skills/palace-tests/` (stdlib `unittest`) exercise resolve/validate/render/staleness/archive
  against throwaway fixture vaults, so refactors can't silently break a hook. They run the repo's hooks,
  not the deployed copies, so a hand edit in `~/.claude` cannot make the suite pass for the repo.
...
- **Secret masking**: `scan-secrets.py --redact` rewrites vendor-prefixed tokens (GitHub, GitLab,
  Anthropic, OpenAI, Slack, AWS, JWT, PEM keys) as `[REDACTED:<rule>]` in both transcript dirs, right
  after each Stop writes them and before `qmd update` indexes them. Wings are only reported, by the daily
  job, never rewritten. The raw JSONL under `~/.claude/projects/` is out of reach and keeps the value.
